...
| `/blob/files/{identifier}`          | PATCH  | Used to change the filename with given {id}                                                 | `bucketID`, `creationTime`, `method`, `sig`           |                                                     | at least one of the optional body parameters | `file`, `fileName`, `additionalMetadata`, `additionalType`, `existsUntil`, `notifyEmail` |
