...
| `/blob/files`                       | POST   | Used to create a file                                                                       | `bucketID`, `creationTime`, `prefix`, `method`, `sig` | `notifyEmail`, `retentionDuration`, `type`          | `file`, `fileName`,                          | `metadata`, `fileHash`, `metadataHash` |
...
| `/blob/files/{identifier}`          | PATCH  | Used to change the filename with given {id}                                                 | `bucketID`, `creationTime`, `method`, `sig`           | `type`, `existsUntil`, `notifyEmail`                | at least one of the optional body parameters | `file`, `fileName`, `metadata`         |
...
| `metadata` (was called `additionalMetadata` until `v0.1.35`) | some additional data the uploader wants to add                                                                                                         | string / json | all valid json strings           |
...
| `blob:create-file-bad-metadata`                    | 400         | The parameter `metadata` is not a valid json                                                                                             |                    |                                  |
...
| `blob:patch-file-data-type-mismatch`             | 400         | The given `metadata` does not match the JSON schema defined in `type`                                                         |                    |         |
